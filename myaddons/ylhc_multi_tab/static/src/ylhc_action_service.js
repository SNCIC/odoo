...
function makeActionManager(env, router = _router) {

    let ylhcStacks = {};
    let currentActionJsId = null;

    const breadcrumbCache = {};
    const keepLast = new KeepLast();
    let id = 0;
    // replace with ylhcStacks
    // let controllerStack = [];
    let dialogCloseProm;
    let actionCache = {};
    let dialog = null;
    ...
    /**
     * Returns the current controller stack.
     */
    function _getCurrentControllerStack() {
        if (!currentActionJsId) {
            return [];
        } else {
            return _getControllerStack(currentActionJsId);
        }
    }
    ...
    /**
     * get controller stack by action js id
     * @param {*} actionJsId 
     * @returns 
     */
    function _getControllerStack(actionJsId) {
        if (actionJsId) {
            return ylhcStacks[actionJsId] || (ylhcStacks[actionJsId] = []);
        } 
        return null
    }
    ...
    function _getCurrentController() {
        if (currentActionJsId) {
            const stack = _getControllerStack(currentActionJsId);
    ...
    function setChangingTab(state) {
        changing_tab = state;
    }

    function isChangingTab() {
        return changing_tab;
    }
    ...
    function _getView(viewType) {
        const controllerStack = _getCurrentControllerStack();
    ...
    function _getSwitchViewParams() {
        ...
        if (state.action && !actionRegistry.contains(state.action)) {
            // need check action id
            const _controllerStack = _getControllerStack(state.action);
            const currentController = _controllerStack[_controllerStack.length - 1];
    ...
    async function _updateUI(controller, options = {}) {
        ...
        let oldActionJsId = currentActionJsId;
        
        let controllerStack = _getCurrentControllerStack();
            ...
            onMounted() {
                if (action.target === "new") {
                    ...
                    if (action.ref_action) {
                        ylhcStacks[action.ref_action.jsId] = nextStack;
                        currentActionJsId = action.ref_action.jsId;
                    } else {
                        ylhcStacks[action.jsId] = nextStack;
                        currentActionJsId = action.jsId;
                    }
                    pushState();
        ...
        if (index != 0) {
            if (action.tag != 'menu') {
                controller.action.ref_action = ylhcStacks[oldActionJsId][0].action
            }
        }
    ...
    function pushState(cStack = _getCurrentControllerStack()) {
        if (!cStack.length) {
            return;
        }
    ...
    return {
        ...
        removeStacks: (jsIds) => {
            for (var i = 0; i < jsIds.length; i++) {
                var jsId = jsIds[i];
                delete ylhcStacks[jsId];
            }
        },

        setCurrentAction: (actionJsId) => {
            currentActionJsId = actionJsId;
